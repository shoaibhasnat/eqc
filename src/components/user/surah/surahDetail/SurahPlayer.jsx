...
// Helper function to extract video_id from video_link or iframe
const extractVideoId = (video) => {
  ...
  if (video.iframe) {
    const match = video.iframe.match(/embed\/([^?&]+)/);
    if (match) return match[1];
  }
  return null;
};

export default function SurahVideoPlayer() {
  ...
  const surahData = useMemo(() => {
    ...
    const processedVideos = (surah.videos || []).map((video) => ({
      ...video,
      video_id: extractVideoId(video),
    })).filter((video) => video.video_id); // Filter out videos without valid IDs
  ...
  // Update selected video when surah data changes
  useEffect(() => {
    if (surahData.videos && surahData.videos.length > 0) {
      setSelectedVideo(surahData.videos[0]);
    }
  }, [surahData]);
        ...
        >
          ...
          >
            <CardContent sx={{ p: { xs: 4, md: 6 }, textAlign: "center" }}>
              ...
              >
                We are working hard to bring you the complete video lectures for this surah. 
  ...
  return (
    ...
    >
      ...
      >
        <Box height={"100%"}  sx={{ 
          borderRadius: 4,flexGrow: 1, overflow: "hidden"}} >
          <iframe
      ...
      >
        ...
        >
          ...
          <Typography
            ...
            sx={{ fontSize: "0.85rem" }}
          >
            {surahData.videos.length} {surahData.videos.length === 1 ? "video" : "videos"}
          </Typography>
        </Box>
        ...
        >
          ...
          {surahData.videos.map((video, index) => {
            const isSelected =
